...
--     local hasXTHaters = Targeting.GetXTHaterCount() > 0
--     local isMA = Core.IAmMA()
--     local hasForceTarg = Globals.ForceTargetID > 0
--     local followingMark = Config:GetSetting('FollowMarkTarget')
--     local backingOff = Globals.BackOffFlag

--     Logger.log_verbose("FindAutoTargetCheck(XTHaters: %s, IamMA: %s, ForceTarget: %s, FollowMark: %s, BackOff: %s)", Strings.BoolToColorString(hasXTHaters),
--         Strings.BoolToColorString(isMA), Strings.BoolToColorString(hasForceTarg), Strings.BoolToColorString(followingMark), Strings.BoolToColorString(backingOff))

--     if backingOff then return false end
--     if hasXTHaters or isMA or hasForceTarg or followingMark then return true end

--     -- check if our MA has a valid target for us.
--     local assistTarg = false
--     if Config:GetSetting('UseAssistList') and not Core.IAmMA() and Globals.MainAssist:len() > 0 then
