...
function Casting.AutoMed()
    ...
    if me.Sitting() and not Casting.Memorizing then
        if Targeting.GetXTHaterCount() > 0 and (Config:GetSetting('DoMed') ~= 3 or Config:GetSetting('DoMelee') or
                ((Config:GetSetting('MedAggroCheck') and Targeting.IHaveAggro(Config:GetSetting('MedAggroCheckPct'))))) then
            Config.Globals.InMedState = false
