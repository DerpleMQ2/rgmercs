...
function Targeting.GetXTHaterIDsSet(printDebug)
    ...
    for i = 1, xtCount do
        ...
        if xtarg and xtarg.ID() > 0 and not xtarg.Dead() and xtarg.Type():lower() ~= "corpse" and (xtarg.Aggressive() or xtarg.TargetType():lower() == "auto hater" or xtarg.ID() == Globals.ForceTargetID) then
            if printDebug then
