...
function Module:RenderCmdRow(cmd, usage, desc)
	...
	ImGui.TextColored(Globals.Constants.Colors.LightOrange, cmd)
	ImGui.TableNextColumn()
	ImGui.TextColored(Globals.Constants.Colors.LightBlue, usage)
	ImGui.TableNextColumn()
	ImGui.PushStyleColor(ImGuiCol.Text, Globals.Constants.Colors.LightGreen)
...
function Module:RenderConfig(search)
	...
	if ImGui.BeginChild("##FAQCommandContainer", ImVec2(0, 0), bit32.bor(ImGuiChildFlags.Border, ImGuiChildFlags.AlwaysAutoResize, ImGuiChildFlags.AutoResizeY)) then
